...
local data = {
	...
	tibiaCoinId = 22118,  -- ID da Tibia Coin
}
	...
	if cylinder and cylinder:isContainer() then
		for i = 0, cylinder:getSize() - 1 do
			...
			if item:isContainer() then
				...
				-- Converter múltiplos de 10 gold coins em tibia coins
				if item:getId() == ITEM_GOLD_COIN then
					local goldCount = item:getCount()
					local tibiaCoinCount = math.floor(goldCount / 10)
					local remainingGold = goldCount % 10

					if tibiaCoinCount > 0 then
						item:remove()
						if remainingGold > 0 then
							cylinder:addItem(ITEM_GOLD_COIN, remainingGold)
						end
						for j = 1, tibiaCoinCount do
							if not (cylinder:addItem(data.tibiaCoinId, 1)) then
								self:addItem(data.tibiaCoinId, 1)
							end
						end
						conv:setAttribute(ITEM_ATTRIBUTE_CHARGES, conv:getAttribute(ITEM_ATTRIBUTE_CHARGES) - tibiaCoinCount)
						return true
					end
				end
			end
	...
	if player ~= nil then
		...
		if player:getItemCount(itemid) >= 1 then
			if item:hasAttribute(ITEM_ATTRIBUTE_CHARGES) then
				...
				if charges_n >= 1 then
					if player:getItemCount(ITEM_GOLD_COIN) >= 10 then
